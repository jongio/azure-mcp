...
public sealed class OnboardClusterToArcCommand : BaseSubscriptionCommand<ArcConnectOptions>
{
    ...
    public override string Description =>
        "Onboards or connects an existing cluster to Azure Arc by validating the cluster and executing the necessary commands.";
    ...
    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, ParseResult parseResult)
    {
        try
        {
            ...
            var success = await _arcService.OnboardClusterToArcAsync(
                options.ClusterName!,
            ...
            context.Response.Message = success
                ...
                : "Failed to connect cluster to Azure Arc.";

            return context.Response;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to connect AKS Edge Essentials cluster to Azure Arc");
            context.Response.Status = 500;
            context.Response.Message = $"Failed to connect cluster to Azure Arc: {ex.Message}";
            return context.Response;
        }
    }
}
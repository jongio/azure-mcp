...
# Start transcript
$transcriptFile = "$PSScriptRoot\arc-disconnect-$(Get-Date -Format 'yyMMdd-HHmm').txt"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        throw "Arc disconnection failed with exit code: $LASTEXITCODE"
    }
    Start-Sleep -Seconds 10
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        throw "Azure Arc resource deletion failed with exit code: $LASTEXITCODE"
    }
    ...
    Write-Host "`nDisconnection failed: $_" -ForegroundColor Red
    Write-Host "`nCheck the log file: $transcriptFile" -ForegroundColor Yellow
    Start-Sleep -Seconds 15
    throw $_
}
